...
namespace MyComputerRepairShop.DAL
{
    public class MCRSInitializer : System.Data.Entity.DropCreateDatabaseIfModelChanges<MCRSContext>
    {
        protected override void Seed(MCRSContext context)
        {
            ...
            if (context.computerParts.Count() <= 1)
            {
            ...
            if (context.repairJobs.Count() <= 1)
            {
                var repairjob = new List<RepairJob>
                {
                    new RepairJob{ Name="Computer start niet op", Startdate=DateTime.Now, Enddate=DateTime.Now, Status=Status.Pending, Detail="Het computer start niet, fans draaien wel.", Client=context.clients.FirstOrDefault(r=>r.Id==1), Worker=context.workers.FirstOrDefault(r=>r.Id==1) },
                    new RepairJob{ Name="Scherm defect", Startdate=DateTime.Now, Enddate=DateTime.Now, Status=Status.Pending, Detail="Het scherm valt tijdens gebruik uit",Client=context.clients.FirstOrDefault(r=>r.Id==2), Worker=context.workers.FirstOrDefault(r=>r.Id==2) }
                };
